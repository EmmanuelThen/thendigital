...
const AccountTabs = ({ firstTab, secondTab, firstTabContent, secondTabContent }: AccountInterface) => {
    ...
    return (
        <Tabs.Root
            className="rounded-lg border border-1 flex flex-col md:w-[500px] shadow-lg"
            defaultValue="tab1"
        >
            <Tabs.List className="shrink-0 flex border-b border-mauve6" aria-label="Manage your account">
                <Tabs.Trigger
                    className="hover:cursor-pointer bg-white px-5 h-[45px] flex-1 flex items-center justify-center text-xs md:text-[15px] leading-none text-mauve11 select-none first:rounded-tl-md last:rounded-tr-md hover:text-blue9 data-[state=active]:text-blue9 data-[state=active]:shadow-[inset_0_-1px_0_0,0_1px_0_0] data-[state=active]:shadow-current data-[state=active]:focus:relative outline-none cursor-default"
                    value="tab1"
                >
                    {firstTab}
                </Tabs.Trigger>
